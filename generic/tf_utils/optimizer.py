...
def create_multi_gpu_optimizer(networks, config, finetune=list(), optim_cst=tf.compat.v1.train.AdamOptimizer):
    ...
    avg_loss = tf.reduce_mean(input_tensor=tf.stack(losses))
    avg_accuracy = tf.reduce_mean(input_tensor=tf.stack(accuracies))
