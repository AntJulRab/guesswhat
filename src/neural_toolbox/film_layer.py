...
class FiLMResblock(object):
    def __init__(self, features, context, is_training,
        ...
        with tf.compat.v1.variable_scope("FiLM", reuse=reuse):
...
if __name__ == '__main__':
    ...
    feature_maps = tf.compat.v1.placeholder(tf.float32, shape=[None, 3, 3, 2])
    lstm_state = tf.compat.v1.placeholder(tf.float32, shape=[None, 6])
    ...
    sess.run(tf.compat.v1.initialize_variables(tf.compat.v1.trainable_variables()))
