...
def batch_norm(input, gammas, betas, epsilon, is_training):
    ...
    # apply batch norm
    inv = gammas * tf.expand_dims(tf.math.rsqrt(variances + epsilon), 0)
...
if __name__ == '__main__':
    ...
    inp = tf.compat.v1.placeholder(tf.float32, [50, 5, 5, 10])
    ...
    mean, variance = tf.compat.v1.global_variables()[:2]
