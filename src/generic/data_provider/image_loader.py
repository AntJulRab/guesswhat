...
#the features are a dictionary with keys: ['features','idx2img']
#h5_file['features'] is an array of shape (46794, 1000)
#h5_file['idx2img'] is an array of shape (46794,)
...
class h5FeatureBuilder(AbstractImgBuilder):
    ...
    def build(self, image_id, filename, optional=True, which_set=None,**kwargs):
        ...
        if self.bufferize:
            ...
            #what does "optional" do? this parameter is never updated! 
            #moreover, if "not optional" is True and (image_id in img2idx) is False we still enter the loop! but then we evaluate img2idx on a key that is not there!
            #
            #before editing the code there was a check: if (optional and image_id in img2idx) or (not optional):

            if image_id in img2idx:
