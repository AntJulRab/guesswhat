...
class AbstractNetwork(object):
    ...
    def get_parameters(self, finetune=None):
        return [v for v in tf.compat.v1.trainable_variables() if self.scope_name in v.name]
...
class ResnetModel(AbstractNetwork):
    ...
    def get_parameters(self, finetune=list()):
        ...
        if len(finetune) > 0:
            for e in finetune:
                fine_tuned_params = [v for v in tf.compat.v1.trainable_variables() if e in v.name and v not in params]
    ...
    def get_resnet_parameters(self):
        ...
        return [v for v in tf.compat.v1.global_variables() if self.scope_name in v.name and
