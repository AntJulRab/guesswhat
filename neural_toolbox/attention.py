...
def compute_attention(feature_maps, context, no_mlp_units, reuse=False):
    with tf.compat.v1.variable_scope("attention"):
        ...
        if len(feature_maps.get_shape()) == 3:
            h = tf.shape(input=feature_maps)[1]  # when the shape is dynamic (attention over lstm)
            w = 1
        ...
        soft_attention = tf.reduce_sum(input_tensor=soft_attention, axis=1)
...
def compute_glimpse(feature_maps, context, no_glimpse, glimpse_embedding_size, keep_dropout, reuse=False):
    with tf.compat.v1.variable_scope("glimpse"):
        h = int(feature_maps.get_shape()[1])
        w = int(feature_maps.get_shape()[2])
        c = int(feature_maps.get_shape()[3])
        ...
        with tf.compat.v1.variable_scope("glimpse"):
            ...
            g_feature_maps = tf.nn.dropout(g_feature_maps, 1 - (keep_dropout))
            ...
            hadamard = tf.nn.dropout(hadamard, 1 - (keep_dropout))
            ...
            for i in range(no_glimpse):
                ...
                soft_glimpses = tf.reduce_sum(input_tensor=soft_glimpses, axis=1)
